...
class ObjPerdu:
    ...
    def init_proba_uniform(self) -> None:
    ...
    def init_proba_center(self) -> None:
    ...
    def maj_grille_prob(self, position: tuple[int, int]) -> None:
        """MAJ de la grille contenant des proba après visite d'une case et si le senseur a renvoyé 0.

        Args:
            position: (ligne, colonne ) de la case visité
        """
    ...
    def tmp_sum(self) -> float:
        """Calcule la somme des valeurs (des probabilités) de chaque case de la grille.
            Nécessaire pour vérifier que la somme de toutes les probas vaut 1.

        Returns:
            Somme des probabilités de chaque case de la grille.
        """
    ...
    def max_proba_grille(self, old_pos: tuple[int, int] = (0, 0)) -> tuple[int, int]:  # done
        ...
        Returns:
            Nouvelle position (ligne, col) de la case avec proba max
        """
